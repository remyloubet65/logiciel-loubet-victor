...
type Entreprise = {
  id?: number
  user_id: string
  nom: string; adresse: string; telephone: string; email: string; siret: string
...
function Dashboard({ userId, email }:{ userId:string; email:string }){
  ...
  async function newDossier(){
    ...
    const d: Dossier = {
      ...
      defunt_nom:'', defunt_prenom:'', famille_contact:'', prestations:[], marbrerie:[], autres:[], cree_le:now, modifie_le:now
    }
    const { data: ins, error } = await supabase.from('dossiers').insert(d).select().single()
    if(error){ alert(error.message); return }
    setDossiers(prev=> [ins as Dossier, ...prev])   // ✅ (corrige l’ancien `*prev`)
    setCurrentId((ins as any).id)
  }
  ...
  function openDevisAndPrint(d: Dossier){
    ...
    const rowsPrest = lignesPrest.map(p=>`<tr><td>${escapeHtml(p.nom)}</td><td style="text-align:right">${fmt(p.prix)}</td></tr>`).join('')
    const rowsMarb = d.marbrerie.map(l=>`<tr><td>${escapeHtml(l.nom)} × ${l.qte}</td><td style="text-align:right">${fmt(l.qte*l.pu)}</td></tr>`).join('')
    const rowsAutres = d.autres.map(l=>`<tr><td>${escapeHtml(l.nom)} × ${l.qte}</td><td style="text-align:right">${fmt(l.qte*l.pu)}</td></tr>`).join('')
    const sig = entreprise?.signature_data_url ? `<div><div style="color:#666;font-size:12px">Signature</div><img src="${entreprise.signature_data_url}" alt="signature" style="height:60px"/></div>` : ''
    ...
    const html = `
      ...
      <div style="text-align:right;font-weight:700;margin-top:8">Total TTC : ${fmt(totalDossier(d))}</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
            <div>
              ...
              <div style={{marginTop:8,maxHeight:'60vh',overflow:'auto',display:'grid',gap:8}}>
                {filtered.map(d=>(
                  <button key={String(d.id)} style={{textAlign:'left',padding:8,border:'1px solid #eee',borderRadius:8,background:'white'}}
              ...
              {current ? (
                <div style={{display:'grid',gap:10}}>
                  ...
                  <div>
                    ...
                    <div style={{textAlign:'right',marginTop:6}}><strong>Sous-total prestations : {currency(totalPrestations(current))}</strong></div>
                  </div>

                  <EditableLines title="Marbrerie" lines={current.marbrerie} onChange={lines=>updateCurrent({ marbrerie: lines } as any)} />
                  <EditableLines title="Autres lignes" lines={current.autres} onChange={lines=>updateCurrent({ autres: lines } as any)} />
          ...
          <div style={{display:'grid',gap:8}}>
            ...
            <div>
              ...
              <input type="file" accept="image/*" onChange={e=>{
                ...
                const r = new FileReader()
                r.onload = ()=> saveEntreprisePatch({ signature_data_url: String(r.result) })
                r.readAsDataURL(f)
              }}/>
...
function EditableLines({ title, lines, onChange }:{ title:string; lines:Ligne[]; onChange:(l:Ligne[])=>void }){
  return (
    <div style={{border:'1px solid #eee',borderRadius:8,padding:10,marginTop:10,background:'white'}}>
      <div style={{fontWeight:600}}>{title}</div>
      <table style={{width:'100%',borderCollapse:'collapse',marginTop:8}}>
        <thead><tr><th style={{textAlign:'left'}}>Libellé</th><th>Qté</th><th>PU</th><th>Total</th><th></th></tr></thead>
        <tbody>
          {lines.map(l=>(
            <tr key={l.id}>
              <td><input value={l.nom} onChange={e=>onChange(lines.map(x=> x.id===l.id? { ...x, nom:e.target.value } : x))}/></td>
              <td><input type="number" value={l.qte} onChange={e=>onChange(lines.map(x=> x.id===l.id? { ...x, qte:Number(e.target.value) } : x))}/></td>
              <td><input type="number" value={l.pu} onChange={e=>onChange(lines.map(x=> x.id===l.id? { ...x, pu:Number(e.target.value) } : x))}/></td>
              <td style={{textAlign:'right'}}><strong>{currency(l.qte*(l.pu||0))}</strong></td>
              <td style={{textAlign:'right'}}><button onClick={()=>onChange(lines.filter(x=> x.id!==l.id))}>Supprimer</button></td>
            </tr>
          ))}
        </tbody>
      </table>
